import { renderBlockWidgets } from './base-renderer.ts'
import { type SyntaxNode, type SyntaxNodeRef } from '@lezer/common'
import { WidgetType, EditorView } from '@codemirror/view'

// KaTeX will be loaded via CDN in the HTML file
declare global {
  interface Window {
    katex: any;
  }
...
import clickAndSelect from './click-and-select.ts'
import { equationMenu } from './context-menu/equation-menu.ts'
...
class MathWidget extends WidgetType {
  ...
  toDOM (view: EditorView): HTMLElement {
    ...
    if (window.katex) {
      window.katex.render(this.equation, elem, { throwOnError: false, displayMode: this.displayMode })
    } else {
      // Fallback if KaTeX is not yet loaded
      elem.textContent = `$${this.displayMode ? '$' : ''}${this.equation}$${this.displayMode ? '$' : ''}`
    }
  ...
  override updateDOM (dom: HTMLElement, _view: EditorView): boolean {
    ...
    if (window.katex) {
      window.katex.render(this.equation, dom, { throwOnError: false, displayMode: this.displayMode })
    } else {
      // Fallback if KaTeX is not yet loaded
      dom.textContent = `$${this.displayMode ? '$' : ''}${this.equation}$${this.displayMode ? '$' : ''}`
    }
